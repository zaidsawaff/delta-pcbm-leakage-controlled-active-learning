...
INPUT_ROOT = WORKING / "REVISION_R4D_FROZEN_INPUTS"
TEMP_ROOT = WORKING / "REVISION_R4D_TEMP"
PROGRESS_ROOT = (
    WORKING
    / "DELTA_REVIEWER_REVISION"
    / "Revision_R4D_LDA_Deterministic_Imbalance_Progress"
)
FINAL_ROOT = (
    WORKING
    / "DELTA_REVIEWER_REVISION"
    / "Revision_R4D_LDA_Deterministic_Imbalance_Final"
)
PROGRESS_PACKET = WORKING / "revision_R4D_lda_deterministic_imbalance_progress_packet.zip"
FINAL_PACKET = WORKING / "revision_R4D_lda_deterministic_imbalance_packet.zip"
REMOTE_OUTPUT = (
    engine.REMOTE_BASE
...
REVISION_R4C_PACKET_SHA256 = (
    "f00e3b761223375551e8672c6d5dbec589fb7b75ccc98751168ffaab80ada22c"
)
REVISION_R2A_PACKET_SHA256 = (
    "554270b1d5bcc0bf7791020b64c44b72a267d71a2ee24afb5f43029957d01a8a"
)
REVISION_PROTOCOL_SHA256 = engine.REVISION_PROTOCOL_SHA256
STAGE = "R4D"
CLASSIFIER = "LDA_SHRINKAGE_LSQR_AUTO"
LDA_NUMERICAL_CONTRACT = (
    "FLOAT32_HISTORY_NORMALIZATION_AND_TRAINING_ARRAYS__"
    "SKLEARN_LDA_LSQR_AUTO_SHRINKAGE_FLOAT64_MODEL__"
    "REPETITION_MEAN_DECISION_SCORES__NATIVE_POSTERIOR"
)
...
MAX_RUNTIME_HOURS = float(os.environ.get("R4D_MAX_RUNTIME_HOURS", "10.0"))
MAX_RUNTIME_SECONDS = MAX_RUNTIME_HOURS * 3600.0
SHARD_STOP_RESERVE_SECONDS = float(
    os.environ.get("R4D_SHARD_STOP_RESERVE_MINUTES", "25")
) * 60.0
CPU_WORKERS = max(
    1, min(int(os.environ.get("R4D_CPU_WORKERS", "4")), os.cpu_count() or 1)
)
BATCH_SIZE = max(CPU_WORKERS, int(os.environ.get("R4D_BATCH_SIZE", "8")))
FORCE_SEPARATE_AGGREGATION_AFTER_SECONDS = float(
    os.environ.get("R4D_FORCE_SEPARATE_AGGREGATION_AFTER_HOURS", "2.0")
) * 3600.0
SHARD_PACKET_PATTERN = re.compile(
    r"^(?P<shard_id>R4D_[A-Za-z0-9_]+)__(?P<sha256>[0-9a-f]{64})\.zip$"
)

SHARD_TABLES = {
    "selections": "revision_R4D_shard_selection_trace.csv",
    "candidate_audits": "revision_R4D_shard_candidate_score_audit.csv",
    "step_audits": "revision_R4D_shard_sequential_step_audit.csv",
    "selector_calls": "revision_R4D_shard_selector_call_audit.csv",
    "oof_rows": "revision_R4D_shard_probability_contract_audit.csv",
    "normalizers": "revision_R4D_shard_normalizer_audit.csv",
    "folds": "revision_R4D_shard_fold_metrics.csv",
    "predictions": "revision_R4D_shard_repetition_predictions.csv",
    "recalls": "revision_R4D_shard_per_class_recall.csv",
    "confusions": "revision_R4D_shard_confusion_matrices_long.csv",
    "coverage": "revision_R4D_shard_selected_class_distribution.csv",
}
...
def resolve_inputs():
    ...
    r4c_packet, r4c_source = engine.resolve_packet(
        "revision_R4C_ridge_random_imbalance_packet.zip",
        REVISION_R4C_PACKET_SHA256,
    )
    r2a_packet, r2a_source = engine.resolve_packet(
    ...
    r4c_report = engine.read_json_member(r4c_packet, "revision_R4C_final_report.json")
    r2a_report = engine.read_json_member(r2a_packet, "revision_R2A_migration_report.json")
    if not r4a_report.get("all_readiness_gates_passed", False):
        raise RuntimeError("Revision R4A parent gates did not pass")
    if not r4c_report.get("all_readiness_gates_passed", False):
        raise RuntimeError("Revision R4C parent gates did not pass")
    if not r2a_report.get("all_readiness_gates_passed", False):
        raise RuntimeError("Revision R2A parent gates did not pass")
    if r4a_report.get("revision_protocol_sha256") != REVISION_PROTOCOL_SHA256:
        raise RuntimeError("Revision R4A protocol hash drift")
    if r4c_report.get("revision_protocol_sha256") != REVISION_PROTOCOL_SHA256:
        raise RuntimeError("Revision R4C protocol hash drift")
    if r2a_report.get("revision_protocol_sha256") != REVISION_PROTOCOL_SHA256:
        raise RuntimeError("Revision R2A protocol hash drift")

    discovery = engine.read_csv_member(r2a_packet, "revision_R2A_packet_discovery.csv")
    lda_rows = discovery.loc[
        discovery["packet"].astype(str).eq("stage3e2a_lda_deterministic_packet.zip")
    ]
    resolved_value = str(lda_rows.iloc[0]["resolved"]).strip().lower() \
        if len(lda_rows) == 1 else "false"
    if len(lda_rows) != 1 or resolved_value not in {"true", "1", "yes"}:
        raise RuntimeError("R2A does not resolve exactly one Stage 3E2A LDA packet")
    lda_expected = str(lda_rows.iloc[0]["expected_sha256"]).lower()
    if re.fullmatch(r"[0-9a-f]{64}", lda_expected) is None:
        raise RuntimeError("Invalid Stage 3E2A hash in R2A")
    lda_packet, lda_source = engine.resolve_packet(
        "stage3e2a_lda_deterministic_packet.zip", lda_expected
    )

    records = []
    for packet, expected, source in [
        (r4a_packet, REVISION_R4A_PACKET_SHA256, r4a_source),
        (r4c_packet, REVISION_R4C_PACKET_SHA256, r4c_source),
        (r2a_packet, REVISION_R2A_PACKET_SHA256, r2a_source),
        (lda_packet, lda_expected, lda_source),
    ]:
    ...
    audit = pd.concat([parents["audit"], pd.DataFrame(records)], ignore_index=True)
    if not audit[["hash_matches", "crc_passes"]].all().all():
        raise RuntimeError("Revision R4D frozen-input integrity failed")
    parents.update(
        {
            "r4a_packet": r4a_packet,
            "r4a_report": r4a_report,
            "r4c_packet": r4c_packet,
            "r4c_report": r4c_report,
            "r2a_packet": r2a_packet,
            "r2a_report": r2a_report,
            "lda_packet": lda_packet,
            "lda_packet_sha256": lda_expected,
            "audit": audit,
...
def fit_history_state_lda(features, main_valid, metadata, history_rows):
    """Locked LDA sensitivity engine: same causal normalization, LDA only."""
    ...
    if history_meta["fixed_test_never_query"].astype(bool).any():
        raise RuntimeError("Fixed test entered LDA history")
    if not history_meta["eligible_for_training"].astype(bool).all():
        raise RuntimeError("Non-training-eligible repetition entered LDA history")
    if sorted(history_meta["label"].unique().tolist()) != list(range(engine.CLASSES)):
        raise RuntimeError("LDA history lacks one or more classes")
    ...
    y = np.repeat(history_meta["label"].to_numpy(dtype=int), engine.WINDOWS)
    model = LinearDiscriminantAnalysis(solver="lsqr", shrinkage="auto")
    model.fit(x, y)
    if model.classes_.tolist() != list(range(engine.CLASSES)):
        raise RuntimeError("LDA class order drift")
    return {
        "model": model,
        "means": means,
        "stds": stds,
        ...
        "numerical_engine_contract": LDA_NUMERICAL_CONTRACT,
        ...
        "std_floor_count": len(floored),
        "std_floored_channels": "|".join(map(str, floored)),
        "minimum_raw_std_before_floor": float(raw_stds.min()),
        "classifier": CLASSIFIER,
    }


def lda_native_probabilities(state, features, main_valid, rows):
    rows = np.asarray(rows, dtype=int)
    scores, predicted, margins = engine.score_repetitions(
        state, features, main_valid, rows
    )
    embeddings = r3b.repetition_embeddings(state, features, main_valid, rows)
    probabilities = np.asarray(
        state["model"].predict_proba(embeddings), dtype=np.float64
    )
    if probabilities.shape != (len(rows), engine.CLASSES):
        raise RuntimeError("LDA native probability shape drift")
    if not np.isfinite(probabilities).all():
        raise RuntimeError("Non-finite LDA native probabilities")
    if not np.allclose(probabilities.sum(axis=1), 1.0, atol=1e-10, rtol=0.0):
        raise RuntimeError("LDA native probabilities do not sum to one")
    if not np.array_equal(np.argmax(probabilities, axis=1), predicted):
        raise RuntimeError("LDA posterior and decision-score predictions disagree")
    return probabilities, scores, predicted, margins


def lda_probability_acquisition(
    strategy, features, main_valid, metadata, history_rows, remaining_rows
):
    state, fit_seconds = timed(
        fit_history_state_lda,
        features,
        main_valid,
        metadata,
        history_rows,
    ...
    if strategy == "LEAST_CONFIDENCE":
        ...
        raise ValueError(f"Invalid LDA probability strategy: {strategy}")
    selected_tokens, ordered = selected_result
    selected_set = set(selected_tokens)
    score_lookup = dict(
        zip(
            ordered["opaque_candidate_token"].astype(str),
            ordered["score"].astype(float),
        )
    )
    candidate_rows = []
    for index, token in enumerate(tokens):
        ...
        for label in range(engine.CLASSES):
            row[f"native_lda_probability_{label}"] = float(
    ...
    call = {
        ...
        "full_history_std_floor_count": int(state["std_floor_count"]),
        "state": state,
...
def lda_diversity_acquisition(
    ...
    if strategy == "RBMAL_MARGIN_DIVERSITY":
        selector_frame = r3b.build_rbmal_frame(
            tokens, margins, candidate_embeddings
        )
        selected_result, selector_seconds = timed(
            r3b.select_rbmal,
            selector_frame,
        ...
        initial_distance = r3b.minimum_distance(
            candidate_embeddings, history_embeddings
        )
        exact_schema = selector_frame.columns.tolist() == r3b.RBMAL_SCHEMA
        ...
        exact_schema = selector_frame.columns.tolist() == r3b.CORE_SET_SCHEMA
    else:
        raise ValueError(f"Invalid LDA diversity strategy: {strategy}")
    selected_set = set(selected_tokens)
    candidate_rows = []
    for index, token in enumerate(tokens):
        candidate_rows.append(
            {
                ...
                "raw_predicted_label_internal_audit_only": int(predicted[index]),
                "raw_margin_internal_audit_only": float(margins[index]),
                "initial_minimum_history_distance": float(
                    initial_distance[index]
                ),
    ...
    call = {
        ...
        "selector_seconds": selector_seconds,
        "selector_schema": "|".join(selector_frame.columns),
        "selector_schema_exact": exact_schema,
        "selector_forbidden_column_count": len(
            set(selector_frame.columns).intersection(r3b.FORBIDDEN_COLUMNS)
        ),
        "oof_fold_count": 0,
...
def activate_lda_contract():
    # These are module-object assignments, so all imported selector paths and
    # forked workers use the same locked LDA fit without changing selectors.
    engine.fit_history_state = fit_history_state_lda
    r3c.probability_acquisition = lda_probability_acquisition
    r3c.diversity_acquisition = lda_diversity_acquisition


def validate_frozen_lda_anchor(inputs, features, main_valid, metadata):
    """Reproduce the frozen balanced-pool LDA sensitivity before R4D."""
    packet = inputs["lda_packet"]
    selection_member, frozen_selection_raw = engine.find_csv_member(
        packet, ["selection", "trace"]
    )
    prediction_member, frozen_prediction_raw = engine.find_csv_member(
        packet, ["repetition", "prediction"]
    )
    fold_member, frozen_fold_raw = engine.find_csv_member(packet, ["fold"])
    frozen_selection, selection_columns = engine.normalize_selection_table(
        frozen_selection_raw
    )
    row_by_uid = dict(
        zip(
            metadata["repetition_uid"].astype(str),
    ...
    frozen_fold, fold_columns = engine.normalize_fold_table(frozen_fold_raw)

    observed_selection, observed_prediction, observed_fold, normalizers = (
        engine.run_deterministic_reconstruction(
            features, main_valid, metadata
        )
    )
    selection_comparison = engine.compare_frozen_selections(
        observed_selection, frozen_selection
    )
    prediction_comparison = engine.compare_frozen_predictions(
        observed_prediction, frozen_prediction
    )
    fold_comparison = engine.compare_frozen_folds(observed_fold, frozen_fold)
    frozen_round_membership_available = bool(
        selection_columns.get("query_round") is not None
        and frozen_selection["query_round"].notna().all()
    )
    round_memberships_match_when_available = bool(
        True
        if not frozen_round_membership_available
        else selection_comparison[
            "selected_round_repetition_identity_membership_matches"
        ].astype(bool).all()
    )
    gates = {
        "stage3e2a_packet_hash_matches_r2a": engine.sha256_file(packet)
        == inputs["lda_packet_sha256"],
        "stage3e2a_packet_crc_passes": engine.archive_crc_passes(packet),
        "frozen_selection_identities_are_resolved": bool(
            frozen_selection["sequence_row"].notna().all()
        ),
        "frozen_lda_trajectory_count_is_56": bool(
            observed_fold.groupby(
                ["participant", "strategy", "query_budget"]
            ).ngroups
            == 56
        ),
        "frozen_lda_fold_count_is_280": len(observed_fold) == 280,
        "frozen_lda_selection_identity_sets_match": bool(
            selection_comparison[
                "selected_repetition_identity_set_matches"
            ].fillna(False).all()
        ),
        "frozen_lda_selection_round_memberships_match_when_available": (
            round_memberships_match_when_available
        ),
        "frozen_lda_prediction_join_is_complete": bool(
            prediction_comparison["_merge"].eq("both").all()
        ),
        "frozen_lda_predictions_match": bool(
            prediction_comparison["predicted_label_matches"].all()
        ),
        "frozen_lda_fold_join_is_complete": bool(
            fold_comparison["_merge"].eq("both").all()
        ),
        "frozen_lda_fold_metrics_match_below_1e_12": bool(
            fold_comparison["absolute_difference"].max() < 1e-12
        ),
        "lda_normalizer_statistics_are_float32": bool(
            normalizers["normalizer_mean_dtype"].eq("float32").all()
            and normalizers["normalizer_std_dtype"].eq("float32").all()
    ...
    if failed:
        raise RuntimeError(f"Frozen LDA anchor failed: {failed}")
    return {
        "gates": gates,
        "members": {
        ...
        "frozen_round_membership_available": frozen_round_membership_available,
        "round_membership_comparison_status": (
            "EXACT_MATCH"
            if frozen_round_membership_available
            else "NOT_RECORDED_IN_STAGE3E2A_FROZEN_SELECTION_TABLE"
        ),
        "selection_columns": selection_columns,
        "prediction_columns": prediction_columns,
        "fold_columns": fold_columns,
        "maximum_fold_metric_difference": float(
            fold_comparison["absolute_difference"].max()
        ),
...
def validate_shard(shard, outputs):
    ...
    gates = {
        ...
        "lda_probability_calls_use_native_posterior_without_oof": bool(
            calls.loc[
                calls["strategy"].isin(
        ...
        "lda_probability_audit_has_no_calibration_rows": bool(
            outputs["oof_rows"].empty
        ),
        "lda_native_probabilities_are_finite_and_sum_to_one": bool(
            np.isfinite(
                candidate.loc[
                    candidate["strategy"].isin(
                        ["LEAST_CONFIDENCE", "PREDICTIVE_ENTROPY"]
                    ),
                    [f"native_lda_probability_{label}" for label in range(7)],
                ].to_numpy(float)
            ).all()
            and np.allclose(
                candidate.loc[
        ...
        "all_lda_normalizers_and_models_follow_locked_dtypes": bool(
            np.isfinite(
                normalizers[
...
def write_shard_packet(shard, outputs, gates, runtime_seconds):
    ...
    report = {
        ...
        "revision_r4c_packet_sha256": REVISION_R4C_PACKET_SHA256,
        "revision_r2a_packet_sha256": REVISION_R2A_PACKET_SHA256,
        "stage3e2a_lda_packet_sha256": WORKER_CONTEXT.get(
            "lda_packet_sha256", ""
        ),
        "classifier": CLASSIFIER,
    ...
    atomic_json(report, shard_root / "revision_R4D_shard_report.json")
    manifest = []
    ...
    atomic_csv(
        pd.DataFrame(manifest), shard_root / "revision_R4D_shard_manifest.csv"
    )
...
def initialize_worker_context(
    ...
    WORKER_CONTEXT = {
        ...
        "rbmal_seed": int(rbmal_seed),
        "lda_packet_sha256": str(lda_packet_sha256),
    }
...
def upload_verified_batch(results):
    ...
    local_upload = TEMP_ROOT / f"R4D_UPLOAD_{batch_token}"
    local_verify = TEMP_ROOT / f"R4D_VERIFY_{batch_token}"
    ...
    parallel = str(max(4, min(16, len(expected))))
    try:
        common = [
            "--transfers",
            parallel,
        ...
        engine.rclone(["copy", str(local_upload), pending_remote, *common])
        engine.rclone(["copy", pending_remote, str(local_verify), *common])
        verified = {}
        for basename, result in expected.items():
            ...
            if (
                ...
                or not engine.archive_crc_passes(downloaded)
            ):
        ...
        listing = engine.rclone(
            ["lsf", REMOTE_SHARDS, "--files-only"]
        ).stdout.splitlines()
        missing = sorted(
            set(expected) - set(Path(name).name for name in listing)
        )
        if missing:
            raise RuntimeError(
                f"Verified R4D batch finalization incomplete: {missing[:3]}"
            )
        return verified
    finally:
...
def make_progress_packet(inputs, manifest, completed, duplicates, decision):
    ...
    report = {
        ...
        "revision_r4c_packet_sha256": REVISION_R4C_PACKET_SHA256,
        "revision_r2a_packet_sha256": REVISION_R2A_PACKET_SHA256,
        "stage3e2a_lda_packet_sha256": inputs["lda_packet_sha256"],
        "classifier": CLASSIFIER,
        "lda_numerical_contract": LDA_NUMERICAL_CONTRACT,
        "frozen_lda_anchor_all_gates_passed": bool(
            all(inputs["lda_anchor"]["gates"].values())
        ),
    ...
    atomic_json(report, PROGRESS_ROOT / "revision_R4D_progress_report.json")
    atomic_csv(inputs["audit"], PROGRESS_ROOT / "revision_R4D_input_audit.csv")
    atomic_csv(manifest, PROGRESS_ROOT / "revision_R4D_expected_shards.csv")
    atomic_csv(completed_frame, PROGRESS_ROOT / "revision_R4D_completed_shards.csv")
    atomic_csv(remaining, PROGRESS_ROOT / "revision_R4D_remaining_shards.csv")
    atomic_csv(pd.DataFrame(duplicates), PROGRESS_ROOT / "revision_R4D_duplicate_records.csv")
    atomic_json(
        {
            "gates": inputs["lda_anchor"]["gates"],
            "members": inputs["lda_anchor"]["members"],
            "identity_source": inputs["lda_anchor"]["identity_source"],
            "frozen_round_membership_available": inputs["lda_anchor"][
                "frozen_round_membership_available"
            ],
            "round_membership_comparison_status": inputs["lda_anchor"][
                "round_membership_comparison_status"
            ],
            "maximum_fold_metric_difference": inputs["lda_anchor"][
                "maximum_fold_metric_difference"
            ],
        },
        PROGRESS_ROOT / "revision_R4D_frozen_lda_anchor_report.json",
    )
    atomic_csv(
        inputs["lda_anchor"]["selection_comparison"],
        PROGRESS_ROOT / "revision_R4D_frozen_lda_selection_comparison.csv",
    )
    atomic_csv(
        inputs["lda_anchor"]["prediction_comparison"],
        PROGRESS_ROOT / "revision_R4D_frozen_lda_prediction_comparison.csv",
    )
    atomic_csv(
        inputs["lda_anchor"]["fold_comparison"],
        PROGRESS_ROOT / "revision_R4D_frozen_lda_fold_comparison.csv",
    )
    crc = engine.make_zip(
        PROGRESS_ROOT,
        PROGRESS_PACKET,
    ...
    if not crc or not verified:
        raise RuntimeError("R4D progress packet persistence failed")
    return digest
...
def bulk_restore_shards(completed):
    cache = TEMP_ROOT / "R4D_ALL_SHARDS_CACHE"
    if cache.exists():
        shutil.rmtree(cache)
    cache.mkdir(parents=True)
    print("Bulk-restoring R4D shard packets with 32 transfers...", flush=True)
    engine.rclone(
        [
            "copy",
            REMOTE_SHARDS,
    ...
    expected = {record["remote_basename"] for record in completed.values()}
    missing = sorted(expected - set(local))
    if missing:
        raise RuntimeError(
            f"Bulk R4D shard restore is incomplete; missing {len(missing)} packets"
        )
    return cache, local
...
def download_and_aggregate(inputs, manifest, completed):
    ...
    aggregate_paths = {
        key: FINAL_ROOT / f"revision_R4D_aggregate_{key}.csv"
    ...
    for index, shard in enumerate(manifest.itertuples(index=False), start=1):
        ...
        report = engine.read_json_member(
            packet, "revision_R4D_shard_report.json"
        )
        valid = bool(
            ...
            and report.get("revision_r4c_packet_sha256")
            == REVISION_R4C_PACKET_SHA256
            and report.get("stage3e2a_lda_packet_sha256")
            == inputs["lda_packet_sha256"]
        ...
        for key in aggregate_keys:
            frame = read_csv_from_shard(packet, SHARD_TABLES[key])
            if key == "confusions":
                frame = (
                    frame.groupby(
                        [
    ...
    atomic_csv(
        confusions,
        FINAL_ROOT / "revision_R4D_confusion_matrices_aggregated.csv",
    )
    ...
    tables = [
        (trajectory_summary, "revision_R4D_trajectory_summary.csv"),
        (participant_level, "revision_R4D_participant_level_summary.csv"),
        (able, "revision_R4D_able_bodied_descriptive_summary.csv"),
        (p07, "revision_R4D_p07_descriptive_summary.csv"),
        (coverage_summary, "revision_R4D_class_coverage_entropy_summary.csv"),
        (compute_summary, "revision_R4D_compute_summary.csv"),
        (call_summary, "revision_R4D_selector_compute_summary.csv"),
        (integrity, "revision_R4D_shard_integrity_manifest.csv"),
    ]
    ...
    gates = {
        ...
        "revision_r4c_packet_hash_matches": engine.sha256_file(inputs["r4c_packet"])
        == REVISION_R4C_PACKET_SHA256,
        "revision_r4c_all_gates_passed": bool(
            inputs["r4c_report"].get("all_readiness_gates_passed")
        ),
        "revision_r2a_packet_hash_matches": engine.sha256_file(inputs["r2a_packet"])
        == REVISION_R2A_PACKET_SHA256,
        "revision_r2a_all_gates_passed": bool(
            inputs["r2a_report"].get("all_readiness_gates_passed")
        ),
        "stage3e2a_lda_packet_hash_matches_r2a": engine.sha256_file(
            inputs["lda_packet"]
        )
        == inputs["lda_packet_sha256"],
        "frozen_lda_anchor_all_gates_passed": bool(
    ...
    report = {
        ...
        "classifier": CLASSIFIER,
        "lda_numerical_contract": LDA_NUMERICAL_CONTRACT,
        "frozen_lda_anchor_maximum_fold_metric_difference": inputs[
            "lda_anchor"
        ]["maximum_fold_metric_difference"],
        "frozen_lda_round_membership_comparison_status": inputs[
            "lda_anchor"
        ]["round_membership_comparison_status"],
        ...
        **count_totals,
        ...
        "runtime_minutes": (time.time() - START_TIME) / 60.0,
        "final_decision": (
            "PASS_TO_REVISION_R4E_LDA_RANDOM_IMBALANCE_SHARDS"
            if not failed
            else "REVISION_R4D_FINAL_AUDIT_FAILED"
        ),
    }
    atomic_json(report, FINAL_ROOT / "revision_R4D_final_report.json")
    shutil.copy2(Path(__file__), FINAL_ROOT / "revision_R4D_executed_source.py")
    manifest_rows = []
    ...
    atomic_csv(
        pd.DataFrame(manifest_rows), FINAL_ROOT / "revision_R4D_output_manifest.csv"
    )
    if failed:
        raise RuntimeError(f"Revision R4D final audit failed: {failed}")
...
def create_final_packet():
    ...
    if not crc or not verified:
        raise RuntimeError("R4D final packet persistence failed")
    return digest


def main():
    print("=" * 104)
    print("REVISION R4D — LDA DETERMINISTIC IMBALANCE SHARDS")
    print("=" * 104)
    ...
    activate_lda_contract()
    print("Validating the reconstructed LDA engine against frozen Stage 3E2A...")
    inputs["lda_anchor"] = validate_frozen_lda_anchor(
        inputs, features, main_valid, metadata
    )
    print(
        "Frozen LDA anchor maximum fold metric difference:",
        inputs["lda_anchor"]["maximum_fold_metric_difference"],
    )
    ...
    if len(manifest) != EXPECTED_SHARDS or not manifest["shard_id"].is_unique:
        raise RuntimeError("R4D shard manifest drift")
    if not manifest["execution_family"].astype(str).eq(
        "LDA_DETERMINISTIC_SENSITIVITY"
    ).all():
        raise RuntimeError("R4D execution-family drift")
    if not manifest["classifier"].astype(str).eq("LDA").all():
        raise RuntimeError("R4D classifier manifest drift")
    ...
    with context.Pool(processes=CPU_WORKERS) as pool:
        while len(completed) < EXPECTED_SHARDS:
            ...
            reserve = max(
                SHARD_STOP_RESERVE_SECONDS,
                typical_batch * 1.5,
            )
            if elapsed + reserve >= MAX_RUNTIME_SECONDS:
                break
            pending = manifest.loc[
                ~manifest["shard_id"].isin(set(completed))
            ].head(BATCH_SIZE)
            if pending.empty:
                break
            batch_started = time.time()
            results = pool.map(
                execute_shard_worker,
            ...
            eta_hours = (
                remaining_count
                * float(np.median(seconds_per_shard[-10:]))
                / 3600.0
            )
            print(
                f"BATCH PASS | completed={len(completed):03d}/{EXPECTED_SHARDS} | "
                f"new={len(results):02d} | wall={batch_seconds:.1f}s | "
    ...
    if not all_complete:
        decision = "PARTIAL_PASS_RESUME_REVISION_R4D_SAME_NOTEBOOK"
        ...
        print()
        print("FINAL DECISION: PARTIAL_PASS_RESUME_REVISION_R4D_SAME_NOTEBOOK")
        return

    if (
        newly_completed
        and time.time() - START_TIME >= FORCE_SEPARATE_AGGREGATION_AFTER_SECONDS
    ):
        decision = (
            "PARTIAL_PASS_ALL_SHARDS_COMPLETE_RESUME_R4D_FOR_FINAL_AGGREGATION"
        )
        progress_hash = make_progress_packet(
            inputs, manifest, completed, duplicates, decision
        )
        print()
        print("All 735 R4D shards are complete and verified.")
    ...
    print()
    print("=" * 104)
    print("REVISION R4D — FINAL DESCRIPTIVE SUMMARY")
    print("=" * 104)
    ...
    print("Runtime minutes:", round((time.time() - START_TIME) / 60.0, 3))
    print()
    print("FINAL DECISION: PASS_TO_REVISION_R4E_LDA_RANDOM_IMBALANCE_SHARDS")
